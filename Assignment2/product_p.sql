...
SELECT PC.model, PC.speed AS GHZ, PC.hd AS GB
FROM PC
WHERE PC.price < 1000
...
SELECT Product.maker 
FROM Product
WHERE Product.type = 'laptop', Product.type != 'PC'
...
WHERE Product.maker = Printer.model 
...
SELECT Product.model, CASE
                        WHEN PC.speed > Laptop.speed THEN PC.price
                        ELSE Laptop.price
                     END AS price
FROM Product
LEFT JOIN PC ON Product.model = PC.model
LEFT JOIN Laptop ON Product.model = Laptop.model
WHERE PC.speed = (SELECT MAX(speed) FROM PC) OR Laptop.speed = (SELECT MAX(speed) FROM Laptop)
